...
1. Load TOML config from `~/vwork/config.toml`
...
4. `src/report/prompt.ts` builds the system prompt; `src/report/memory.ts` loads past reports from `~/.vwork/reports/` for continuity context
...
- Custom MCP servers can be added via `vwork mcp add` and are stored in `.mcp.json`
- Auth tokens are stored in `~/vwork/auth/` (GitHub OAuth, Atlassian tokens, Slack tokens)
- Secret resolution (`config.ts:resolveSecret`) treats ALL_CAPS values as env var names, otherwise as literal values
- `src/mcp/catalog.ts` defines the built-in integration catalog (shown during `vwork init`): GitHub, Jira, Slack, Filesystem, Fetch, Brave Search, PostgreSQL, Sentry, Linear, Notion
- `src/memory/` provides optional vector search over past reports via sqlite-vec + Voyage embeddings
- `src/prompts/multiselect.ts` is a custom interactive multiselect UI for `vwork init`
